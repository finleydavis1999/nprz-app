...
#   - Origin-percentage variables — schema changed in 2024 (geboorteland /
#     herkomst vs Westers / NietWesters). Carrying both is messy. Defer to
#     follow-up; this script does 2024+ origin-percentage only if simple.
#
# To run:
...
#   weight_col    — internal name of the column to weight by, for wmean. The
#                   internal name resolves to whatever column exists in the
#                   spec's `internal` namespace (population, woningen, etc).
#   internal      — short internal name for cross-reference (weight_col uses it)
#   year_cols     — named list: year-string -> KWB source column name, with
#                   the longest year having the canonical column name. Years
#                   not listed -> variable unavailable that year.
#
# year_cols use the per-year suffix patterns observed in the KWB OData tables
# (suffix numbers shift slightly across years; the spec records them honestly).
.kwb_spec <- function() {
  tribble(
...
# Map each spec variable to the actual source column for a given year.
# Returns a vector of values for that variable from kwb_raw, or NA if the
# year doesn't have it. Year-suffix patterns recorded honestly here; the
# longest year-fallback chain covers a single variable across 6 KWB versions.
.kwb_value_for <- function(internal_name, kwb_raw, year_int) {
  cands <- switch(
    internal_name,
...
.kwb_wide_for_year <- function(kwb_raw, year_int) {
  ...
  for (i in seq_len(nrow(spec))) {
    s <- spec[i, ]
    df[[s$internal]] <- .kwb_value_for(s$internal, kwb_raw, year_int)
  }
...
build_cbs_kwb <- function() {
  ...
  out_dir <- "static/data/parquet"
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  buurt_path <- file.path(out_dir, "cbs-kwb-buurt.parquet")
  gem_path   <- file.path(out_dir, "cbs-kwb-gem.parquet")
  ...
  for (yr in names(.KWB_YEARS)) {  # ascending: list iterates in insertion order
    ...
    all_buurt <- bind_rows(buurt_accum)
    all_gem   <- bind_rows(gem_accum)
    arrow::write_parquet(all_buurt, buurt_path, compression = "zstd", compression_level = 3)
    arrow::write_parquet(all_gem,   gem_path,   compression = "zstd", compression_level = 3)
    cat(sprintf("  wrote %s (%d rows) + %s (%d rows)\n",
                basename(buurt_path), nrow(all_buurt),
                basename(gem_path),   nrow(all_gem)))
  }
...
# Manifest helpers — call these to print manifest JSON for the app.
.kwb_manifest <- function(scale_label, parquet_filename) {
  spec <- .kwb_spec()
  list(
    name = sprintf("CBS Kerncijfers Wijken en Buurten (%s)", scale_label),
    description = "Multi-year (2015-2025) KWB at buurt resolution, long-format.",
    scales = setNames(list(parquet_filename), tolower(scale_label)),
    fields = list(
      year = list(
...
manifest_kwb_buurt <- function() .kwb_manifest("Buurt", "parquet/cbs-kwb-buurt.parquet")
manifest_kwb_gem   <- function() .kwb_manifest("Gemeente", "parquet/cbs-kwb-gem.parquet")